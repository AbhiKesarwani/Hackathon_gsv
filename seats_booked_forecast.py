...
needs_differencing = check_stationarity(df_daily['Seats_Booked'])

# Step 4: Plot ACF & PACF to determine (p, q)
fig, axes = plt.subplots(1, 2, figsize=(12, 5))

# ACF Plot
sm.graphics.tsa.plot_acf(df_daily['Seats_Booked'].diff().dropna() if needs_differencing else df_daily['Seats_Booked'], 
                          lags=40, ax=axes[0])
axes[0].set_title("Autocorrelation Function (ACF)")

# PACF Plot
sm.graphics.tsa.plot_pacf(df_daily['Seats_Booked'].diff().dropna() if needs_differencing else df_daily['Seats_Booked'], 
                           lags=40, ax=axes[1])
...
plt.plot(df_daily['Date'], df_daily['Seats_Booked'], label="Actual Data", color="blue")
